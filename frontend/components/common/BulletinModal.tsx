...
const BulletinModal: React.FC<BulletinModalProps> = ({
	...
	// Sync state to props during render to avoid useEffect warnings
	if (isOpen !== prevIsOpen || initialDate !== prevInitialDate) {
		setPrevIsOpen(isOpen);
		setPrevInitialDate(initialDate);
		if (isOpen) {
			const idx = initialDate
				? bulletins.findIndex((b) => b.date === initialDate)
				: 0;
							...
							<div className="flex items-center space-x-3">
								...
								<div>
									<h3 className="text-lg md:text-xl font-bold text-stone-800">
										{currentBulletin.date} 주보
									</h3>
									<p className="text-[10px] md:text-xs text-stone-400 uppercase font-eng tracking-wider">
						...
						<div className="flex-1 flex flex-col md:flex-row overflow-hidden bg-stone-50">
							...
							<div className="flex-1 overflow-y-auto p-4 md:p-10 space-y-8 flex flex-col items-center">
								{currentBulletin.images.map((img, idx) => (
									<div
										key={idx}
										className="w-full max-w-3xl bg-white shadow-lg rounded-2xl overflow-hidden border border-stone-200"
									>
										<Image
											src={img}
											alt={`${currentBulletin.date} Page ${idx + 1}`}
											width={0}
											height={0}
											sizes="100vw"
									...
									{bulletins.map((item, idx) => (
										...
										>
											<div className="text-left">
												<p
													className={`text-[10px] font-eng mb-0.5 ${selectedIdx === idx ? "text-emerald-600 font-bold" : "text-stone-400"}`}
												>
													{item.date}
												</p>
												<p
													className={`font-bold text-sm ${selectedIdx === idx ? "text-emerald-900" : "text-stone-700"}`}
												>
													{item.title}
												</p>
											</div>
											{selectedIdx === idx ? (
												...
												<ChevronRight
													size={16}
													className="text-stone-300 group-hover:text-emerald-500 group-hover:translate-x-1 transition-all"
												/>
											)}
										</button>
									))}
