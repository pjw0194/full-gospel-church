...
export default function AboutPage() {
  ...
  return (
    <main className="animate-in fade-in duration-700">
      {/* Page Header */}
      <section className="bg-emerald-50 pt-32 sm:pt-40 pb-16 sm:pb-24 relative overflow-hidden">
      ...
      <section className="py-16 sm:py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* Mobile/tablet: float layout (text wraps around image) | Desktop (lg+): 2-col grid */}
          <div className="overflow-hidden lg:overflow-visible lg:grid lg:grid-cols-2 lg:gap-20 lg:items-center">
            {/* Image column: float on mobile, normal grid item on desktop */}
            <div className="relative float-left lg:float-none mr-6 sm:mr-10 mb-4 lg:mb-0 lg:mr-0 w-44 sm:w-60 lg:w-full">
              {/* Decorative circles — desktop only */}
              <div className="hidden lg:block absolute -top-10 -left-10 w-40 h-40 bg-emerald-50 rounded-full -z-10 animate-pulse" />
              <div className="hidden lg:block absolute -bottom-10 -right-10 w-60 h-60 bg-stone-50 rounded-full -z-10" />

              {/* Portrait */}
              <div className="relative w-full aspect-[3/4] rounded-2xl sm:rounded-3xl shadow-lg sm:shadow-2xl z-10 overflow-hidden">
                <Image
                  src="/images/members/권세열.jpg"
                  alt="Pastor Greeting"
                  fill
                  className="object-cover object-top"
                  sizes="(max-width: 640px) 176px, (max-width: 1024px) 240px, 45vw"
                  priority
                />
              </div>

              {/* Mobile/tablet name card — below image, in flow */}
              <div className="lg:hidden mt-3 bg-white/90 backdrop-blur-md px-4 py-3 rounded-2xl shadow border border-emerald-100">
                <p className="text-stone-400 text-xs mb-0.5">Senior Pastor</p>
                <p className="text-sm font-bold text-[#333]">권세열 담임목사</p>
              </div>

              {/* Desktop name card — absolute overlay on image */}
              <div className="hidden lg:block absolute bottom-6 right-4 bg-white/90 backdrop-blur-md px-5 py-4 rounded-2xl shadow-lg z-20 border border-emerald-100">
                <p className="text-stone-400 text-sm mb-0.5">Senior Pastor</p>
            ...
            <div className="space-y-4 sm:space-y-6 lg:space-y-8">
              ...
              <h2 className="text-lg sm:text-3xl md:text-4xl font-bold text-[#333] leading-tight">
                하나님의 은혜와 성령의 능력이
                <br className="hidden sm:block" /> 함께하는 캔사스순복음교회
              </h2>
              <div className="space-y-3 sm:space-y-6 text-stone-600 text-sm sm:text-lg leading-relaxed">
                <p>
      ...
      <section className="py-16 sm:py-24 bg-white overflow-hidden border-t border-stone-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <section className="py-16 sm:py-24 bg-stone-50 border-y border-stone-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            {pastPastors.map((pastor, idx) => (
              <div
                key={idx}
                className="bg-white p-6 rounded-3xl shadow-sm border border-stone-100 flex flex-col items-center justify-center text-center group hover:bg-emerald-600 transition-all duration-300"
              >
                ...
                <h4 className="font-bold text-stone-800 mb-1 group-hover:text-white transition-colors">
                  {pastor.name}
                </h4>
                {pastor.period && (
                ...
                {pastor.isFounder && (
                  <span className="mt-3 text-[9px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full group-hover:bg-white group-hover:text-emerald-600">
                    창립 목사
                  </span>
                )}
              </div>
